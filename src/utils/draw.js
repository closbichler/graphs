...
function drawNode(ctx, node, graph) {
    ctx.lineWidth = 2;
    ctx.fillStyle = "#fff"
...
function drawEdge(ctx, node1, node2, edge, graphProperties) {
    ...
    if (graphProperties.directed && edge.directed) {
        let edgeVector = Vector.getVectorFromPoints(node1.pos, node2.pos).getUnitVector();
        let arrowStart = node2.pos.sub(edgeVector.mult(CONSTANTS.nodeSize));
        let arrowEnd = arrowStart.sub(edgeVector.mult(12));
        let arrowRight = arrowEnd.add(edgeVector.getPerpendicularRight().mult(5));
        let arrowLeft = arrowEnd.add(edgeVector.getPerpendicularLeft().mult(5));
    ...
    if (graphProperties.weighted) {
        ...
        let perp = Vector.getVectorFromPoints(node1.pos, node2.pos).getUnitVector().getPerpendicularRight().mult(14);
